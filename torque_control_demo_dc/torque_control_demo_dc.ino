...
void loop() {
  ...
  if (pwm_out < 1) { pwm_out = 1; }; // Prevent quirky behavior at 0 pulse-width
  /*Execute PWM command*/
  if (!DEBUG_MODE) {
  ...
  while (loop_stop < LOOP_PERIOD) {
    ...
    if (LOG_MODE) {
      Serial.print(angle_des, 2);
      Serial.print(" ");
      Serial.print(angle_norm, 2);
      Serial.print(" ");
      Serial.print(angle_des - angle_norm, 2);
      Serial.print(" ");
      // Serial.println(torque_obs);
      Serial.print(torque_norm, 3);
      Serial.print(" ");
      Serial.print(ctrl_state.out);
      Serial.print(" ");
      // Serial.print(PWM_MAX - fabs(ctrl_state.out),0); Serial.print(" ");
      Serial.print(pwm_out, 0);
      Serial.print(" ");
      Serial.println();
    }
